...
export const getFriendsLeaderboard = query({
	...
	handler: async (ctx) => {
		...
		const acceptedFriendCandidates = [...f1, ...f2].reduce<string[]>(
			(acc, f) => {
				if (f.status === "accepted") {
		...
		const blockChecks = await Promise.all(
			acceptedFriendCandidates.map((friendId) =>
				hasBlock(ctx, userId, friendId),
			),
		);
		const acceptedFriends = acceptedFriendCandidates.filter(
			(_, idx) => !blockChecks[idx],
		);
		...
		const userDetails = await Promise.all(
			userIds.map(async (uid) => {
				...
				const upcomingExams = exams.reduce<{ title: string; date: string }[]>(
					(acc, e) => {
...
export const importFriendExam = mutation({
	...
	handler: async (ctx, args) => {
		...
		const match = existingSubject.find(
			(s) => s.name.toLowerCase() === sourceSubject.name.toLowerCase(),
		);
		...
		return await ctx.db.insert("exams", {
			...
			notes: sourceExam.notes
				? `Imported from friend. Original notes: ${sourceExam.notes}`
...
export const getBlockedUsers = query({
	...
	handler: async (ctx) => {
		...
		const results = profiles.reduce<{ userId: string; username: string }[]>(
			(acc, p) => {
				if (p) {
					acc.push({
						userId: p.userId,
