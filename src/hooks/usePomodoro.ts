...
type Action =
  ...
  | { type: "COMPLETE"; nextMode: Mode; nextStatus: Status; nextTimeLeft: number }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "TICK": {
      ...
      return { ...state, timeLeft: state.timeLeft - 1, elapsedSeconds: state.mode === "work" ? state.elapsedSeconds + 1 : state.elapsedSeconds };
    }
    case "COMPLETE":
      return { ...state, mode: action.nextMode, status: action.nextStatus, timeLeft: action.nextTimeLeft, elapsedSeconds: 0 };
    case "RESET":
      return { ...state, status: "idle", mode: "work", timeLeft: action.workDuration * 60, elapsedSeconds: 0 };
  }
}

function init(): State {
  if (typeof window === "undefined") {
    return { status: "idle", mode: "work", workDuration: 25, breakDuration: 5, timeLeft: 1500, elapsedSeconds: 0 };
  }
  ...
  return { status, mode, workDuration, breakDuration, timeLeft, elapsedSeconds };
}
...
export function usePomodoro(): PomodoroApi {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("pomodoroElapsedSeconds", String(state.elapsedSeconds));
  }, [state.status, state.mode, state.workDuration, state.breakDuration, state.timeLeft, state.elapsedSeconds]);
  ...
  const handleComplete = useCallback(() => {
    ...
    if (s.mode === "work") {
      ...
      if (s.breakDuration === 0) {
        dispatch({ type: "COMPLETE", nextMode: "work", nextStatus: "idle", nextTimeLeft: s.workDuration * 60 });
      } else {
        dispatch({ type: "COMPLETE", nextMode: "break", nextStatus: "paused", nextTimeLeft: s.breakDuration * 60 });
      }
    } else {
      playAlertSound("break");
      dispatch({ type: "COMPLETE", nextMode: "work", nextStatus: "paused", nextTimeLeft: s.workDuration * 60 });
    }
  }, []);
  ...
  const displayTimeLeft = state.status === "idle"
    ? (state.mode === "work" ? state.workDuration * 60 : state.breakDuration * 60)
    : state.timeLeft;
