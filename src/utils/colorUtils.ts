...
export function applyThemeCustomizations(theme: "light" | "dark") {
	...
	for (const v of CUSTOMIZABLE_VARIABLES) {
	...
	if (baseRgb) {
		...
		document.documentElement.style.setProperty(
			"--accent-glow",
			`rgba(${baseRgb.r}, ${baseRgb.g}, ${baseRgb.b}, ${defaultGlowOpacity})`,
		);
		document.documentElement.style.setProperty(
			"--accent-glow-subtle",
			`rgba(${baseRgb.r}, ${baseRgb.g}, ${baseRgb.b}, ${defaultSubtleGlowOpacity})`,
		);
	}
	...
	for (const [variable, value] of Object.entries(customizations)) {
		...
		if (variable === "--accent-primary") {
			const hoverColor = adjustColorLightness(
				value,
				theme === "light" ? -8 : 8,
			);
			const evenDarker = adjustColorLightness(
				value,
				theme === "light" ? -15 : -5,
			);
			...
			document.documentElement.style.setProperty(
				"--accent-gradient",
				`linear-gradient(135deg, ${value}, ${hoverColor})`,
			);
			document.documentElement.style.setProperty(
				"--accent-gradient-hover",
				`linear-gradient(135deg, ${hoverColor}, ${evenDarker})`,
			);
		}
		...
		if (variable === "--accent-glow") {
			...
			if (rgb) {
				...
				document.documentElement.style.setProperty(
					"--accent-glow",
					`rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${glowOpacity})`,
				);
				document.documentElement.style.setProperty(
					"--accent-glow-subtle",
					`rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${subtleGlowOpacity})`,
				);
			}
		}
		...
		if (variable === "--bg-secondary") {
			...
			const elevatedColor = adjustColorLightness(
				value,
				theme === "light" ? -4 : 6,
			);
			document.documentElement.style.setProperty(
