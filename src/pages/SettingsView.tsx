      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div className="card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{
              ...
              <span style={{ fontSize: "0.88rem", fontWeight: 600, color: "var(--text-secondary)" }}>{t.settings.emailLabel}</span>
              <span style={{ fontSize: "0.9rem", fontWeight: 700, color: "var(--text-primary)" }}>
                {userEmail === undefined ? t.common.loading : userEmail ?? t.settings.notSet}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="form-group" style={{ maxWidth: 280 }}>
            <label htmlFor="settings-language" style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--text-secondary)", marginBottom: 8, display: "block" }}>
        ...
        <Modal title={t.settings.privacyPolicyTitle} onClose={() => setShowLegal(null)}>
          <div style={{ fontSize: "0.85rem", lineHeight: "1.5", color: "var(--text-secondary)", display: "flex", flexDirection: "column", gap: 12 }}>
            {t.settings.privacyPolicyText.map((p) => (
              <p key={p}>
                {p.startsWith("Effective Date:") || p.startsWith("Date d'effet :") ? (
                  <strong>{p}</strong>
                ) : p.includes(": ") ? (
                  <>
