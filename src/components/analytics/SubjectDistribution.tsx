...
export function SubjectDistribution({
	...
	const distribution = useMemo(
		() => buildDistribution(allLogs, subjects, t.common.uncategorized),
		[allLogs, subjects, t],
	);
		...
		<div className="analytics-donut-layout">
			<div style={{ position: "relative", width: 130, height: 130 }}>
				...
				<svg viewBox="0 0 160 160" width="100%" height="100%">
					{paths.map((slice, idx) => {
						...
						return (
							...
							<circle
								...
								stroke={slice.color}
								strokeWidth={
									isActive ? slice.strokeWidth + 3 : slice.strokeWidth
								}
								...
								style={{
									cursor: "pointer",
									opacity: selectedSubjectId && !isSelected ? 0.35 : 1,
								}}
					...
					>
						{hoveredSlice !== null
							? `${distribution[hoveredSlice].percentage}%`
							: selectedSubjectId
								? `${distribution.find((d) => d.id === selectedSubjectId)?.percentage ?? 0}%`
								: "Total"}
					</text>
					<text
						x="80"
					...
					>
						{hoveredSlice !== null
							? distribution[hoveredSlice].name
							: selectedSubjectId
								? (distribution.find((d) => d.id === selectedSubjectId)?.name ??
									"")
								: t.sidebar.subjects}
					</text>
				</svg>
