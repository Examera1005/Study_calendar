...
function buildChartData(
	...
	return days.map((dateStr) => {
		...
		return {
			dateStr,
			label: shortLabel(dateStr, dateLocale, dateFormatShort),
			minutes,
		};
	});
}
...
function buildChartElements(
	progressionData: { label: string; minutes: number }[],
): ChartElements | null {
...
export function ProgressionChart({
	...
	const _chartWidth = WIDTH - PAD_LEFT - PAD_RIGHT;
	const chartHeight = HEIGHT - PAD_TOP - PAD_BOTTOM;
	...
	return (
		<div style={{ position: "relative", width: "100%" }}>
			...
			>
				...
				{elements.areaData && (
					<path d={elements.areaData} fill="url(#area-grad)" />
				)}

				<line
					x1={PAD_LEFT}
					y1={elements.averageY}
					x2={WIDTH - PAD_RIGHT}
					y2={elements.averageY}
					stroke="var(--warning)"
					strokeWidth="1.5"
					strokeDasharray="4,4"
				/>
				...
				>
					...
					{formatDuration(Math.round(elements.averageMinutes), {
				...
				{elements.medianMinutes > 0 && (
					<g>
						...
						<text
							x={PAD_LEFT + 6}
							y={elements.medianY - 6}
							textAnchor="start"
							fontSize="0.65rem"
							fill="var(--accent-primary)"
							fontWeight="600"
						>
							{t.analytics.medianLabel}:{" "}
							{formatDuration(Math.round(elements.medianMinutes), {
								formatUnderHourAsMins: true,
							})}
						</text>
					</g>
				...
				{elements.points.map((p, idx) => {
					const _prev = idx === 0 ? p : elements.points[idx - 1];
					const isAbove = p.value >= elements.averageMinutes;
					const strokeColor = isAbove ? "var(--success)" : "var(--danger)";
					const shouldShowLabel =
						timeRange === 7 ||
						idx % 5 === 0 ||
						idx === elements.points.length - 1;
					return (
						// biome-ignore lint/suspicious/noArrayIndexKey: Dynamic Convex API / third-party type
						<g key={idx}>
			...
			{hoveredPoint && (
				...
				>
					...
					{formatDuration(hoveredPoint.value)}
				</div>
			)}
		</div>
	);
