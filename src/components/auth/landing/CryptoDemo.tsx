...
export function CryptoDemo() {
	...
	const clearCryptoTimers = useCallback(() => {
		...
		// biome-ignore lint/suspicious/useIterableCallbackReturn: Dynamic Convex API / third-party type
	...
	const startCryptoProcess = () => {
		...
		scrambleIntervalRef.current = window.setInterval(() => {
			setDemoState((prev) => {
				const nextScrambled = prev.scrambled
					.split("")
					.map((c, i) => {
						if (c === " ") return " ";
						if (i < tickCount * 4) {
					...
					scheduleTimeout(() => {
						...
						scrambleIntervalRef.current = window.setInterval(() => {
							setDemoState((prev) => {
								...
								const nextScrambled = originalWords
									.map((char, index) => {
										if (char === " ") return " ";
	...
	return (
		<>
			...
			<h3>{t.landingPage.cryptoTitle}</h3>
			<p className="lp-demo-desc">{t.landingPage.cryptoDesc}</p>
			...
			<div className="lp-demo-interactive">
				<div className="lp-crypto-panel">
					...
					>
						...
						<div className="lp-crypto-bubble" style={{ flex: 1 }}>
							<div className="lp-crypto-bubble-header">
								...
								<span className="lp-crypto-key-badge">
									{t.landingPage.cryptoSenderKey}
								</span>
							</div>
						...
						<div className="lp-crypto-bubble" style={{ flex: 1 }}>
							...
							<div
								className="lp-crypto-text"
								style={{
									color: demoState.stage === "done" ? "#ffffff" : "#71717a",
								}}
							>
					...
					<div className="lp-crypto-pipeline">
						...
						{demoState.stage !== "plain" && (
							<div
								className={`lp-crypto-pipeline-dot ${demoState.stage === "transmitting" ? "animating" : ""}`}
							/>
						)}
						<span className="lp-crypto-pipeline-label">
							{demoState.stage === "plain" && t.landingPage.cryptoStagePlain}
							{demoState.stage === "encrypting" &&
								t.landingPage.cryptoStageEncrypting}
							{demoState.stage === "cipher" && t.landingPage.cryptoStageCipher}
							{demoState.stage === "transmitting" &&
								t.landingPage.cryptoStageTransmitting}
							{demoState.stage === "decrypting" &&
								t.landingPage.cryptoStageDecrypting}
					...
					<div className="lp-crypto-actions">
						<button
							...
							disabled={
								demoState.stage !== "plain" && demoState.stage !== "done"
							}
						>
