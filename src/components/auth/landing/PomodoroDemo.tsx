...
type PomoAction =
	...
	| { type: "RESET" }
	| {
			type: "SET_TRANSLATED_LOGS";
			payload: { id: number; time: string; label: string }[];
	  };
...
export function PomodoroDemo() {
	...
	const [state, dispatch] = useReducer(pomoReducer, {
		minutes: 25,
		running: false,
		logs: [{ id: 1, time: "10:15", label: "MOCK_LOG_CHEMISTRY" }],
	});
	...
	return (
		<>
			...
			<h3>{t.landingPage.demoPomodoroTitle}</h3>
			<p className="lp-demo-desc">{t.landingPage.pomodoroDesc}</p>
				...
				<div className="lp-pomo-flex">
					...
					<div className="lp-pomo-controls">
						<span className="lp-pomo-state-text">
							{pomoRunning
								? t.landingPage.pomodoroActive
								: t.landingPage.pomodoroIdle}
						</span>
						<div style={{ display: "flex", gap: 8 }}>
							<button
								...
								aria-label={
									pomoRunning
										? t.landingPage.pomodoroPause
										: t.landingPage.pomodoroStart
								}
							>
								{pomoRunning
									? t.landingPage.pomodoroPause
