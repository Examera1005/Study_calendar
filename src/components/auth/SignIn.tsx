...
export function SignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        } else if (msg.includes("signUpError") || msg.includes("could not create account")) {
          userFriendlyError = t.auth.signUpError;
        } else {
          userFriendlyError = msg;
        }
      } else {
        userFriendlyError = step === "signIn"
  ...
  return (
    <div className="auth-page">
            ...
            <div className="form-group">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                <label htmlFor="auth-password">
                  {step === "verifyResetCode" ? t.auth.newPasswordLabel : t.auth.passwordLabel}
                </label>
                {step === "signIn" && (
                  <button
                    type="button"
              ...
              <input
                ...
                autoComplete={step === "signUp" ? "new-password" : step === "verifyResetCode" ? "new-password" : "current-password"}
                required
                minLength={8}
              />
            </div>
        ...
        {(step === "signIn" || step === "signUp") && (
          <div style={{ textAlign: "center", marginTop: 16 }}>
            <button
              ...
              className="btn btn-ghost"
              onClick={() => {
                dispatch({ type: "SET_STEP", payload: step === "signIn" ? "signUp" : "signIn" });
              }}
              id="auth-toggle"
            >
              {step === "signIn"
      ...
      {showLegal === "privacy" && (
        <Modal title={t.settings.privacyPolicyTitle} onClose={() => dispatch({ type: "SET_LEGAL", payload: null })}>
      ...
      {showLegal === "terms" && (
        <Modal title={t.settings.termsOfServiceTitle} onClose={() => dispatch({ type: "SET_LEGAL", payload: null })}>
          <div style={{ fontSize: "0.85rem", lineHeight: "1.5", color: "var(--text-secondary)", display: "flex", flexDirection: "column", gap: 12 }}>
