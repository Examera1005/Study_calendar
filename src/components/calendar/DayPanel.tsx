    ...
    <div className="day-panel">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button type="button" className="btn btn-secondary btn-sm" onClick={onOpenAddTask}>
            {t.calendar.addTaskBtn}
          </button>
          <button type="button" className="btn btn-secondary btn-sm" onClick={onOpenAddEvent}>
            {t.calendar.addEventBtn}
          </button>
          <button type="button" className="btn btn-secondary btn-sm" onClick={onOpenAddLog}>
      ...
      <div className="panel-section">
        ...
        {!tasks || tasks.length === 0 ? (
          <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>{t.calendar.noTasks}</p>
        ) : (
          tasks.map((task) => (
      ...
      <div className="panel-section">
        ...
        {!events || events.length === 0 ? (
          <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>{t.calendar.noEvents}</p>
        ) : (
          events.map((ev) => (
      ...
      <div className="panel-section">
        ...
        {!logs || logs.length === 0 ? (
          <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>{t.calendar.noLogs}</p>
        ) : (
          logs.map((log) => {
