...
function getSubject(subjects: Subject[] | undefined, id: string | undefined): Subject | undefined {
    ...
    <div className="day-panel">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button type="button" className="btn btn-secondary btn-sm" onClick={onOpenAddTask}>+ Task</button>
          <button type="button" className="btn btn-secondary btn-sm" onClick={onOpenAddEvent}>+ Event</button>
          <button type="button" className="btn btn-secondary btn-sm" onClick={onOpenAddLog}>+ Log</button>
        </div>
      </div>
          ...
          tasks.map((task) => (
            <div key={task._id} className={`task-item ${task.completed ? "completed" : ""}`}>
              ...
              <div className="item-actions" style={{ display: "flex", gap: 4 }}>
                <button type="button" className="btn-icon" style={{ width: 28, height: 28 }} aria-label={`Edit ${task.title}`} onClick={() => onEditTask(task)}>✏️</button>
      ...
      <div className="panel-section">
        ...
        {!logs || logs.length === 0 ? (
          ...
          logs.map((log) => {
            ...
            return (
              <div key={log._id} className="log-card" style={{ display: "flex", alignItems: "center", gap: 12 }}>
                {subj && <SubjectBadge name={subj.name} color={subj.color} icon={subj.icon} />}
                <div style={{ flex: 1 }}>
                  <div className="log-content">{log.content}</div>
                  {log.duration != null && <div className="log-meta">⏱️ {formatDuration(log.duration, { formatUnderHourAsMins: true })}</div>}
                </div>
                <div className="item-actions" style={{ display: "flex", gap: 4 }}>
                  <button type="button" className="btn-icon" style={{ width: 28, height: 28 }} aria-label={`Edit log`} onClick={() => onEditLog(log)}>✏️</button>
                  <button type="button" className="btn-icon" style={{ width: 28, height: 28 }} aria-label={`Delete log`} onClick={() => void removeLog({ id: log._id })}>🗑</button>
                </div>
              </div>
            );
          })
