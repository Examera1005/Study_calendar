...
export type TimerCorner =
	| "bottom-right"
	| "bottom-left"
	| "top-right"
	| "top-left";

const CORNERS: TimerCorner[] = [
	"bottom-right",
	"bottom-left",
	"top-right",
	"top-left",
...
export function TimerWidgetSettingsCard({
	corner,
	scale,
	onCornerChange,
	onScaleChange,
	...
	const getCornerLabel = (c: TimerCorner) => {
		switch (c) {
			case "bottom-right":
				return `↘ ${t.settings.cornerBottomRight}`;
			case "bottom-left":
				return `↙ ${t.settings.cornerBottomLeft}`;
			case "top-right":
				return `↗ ${t.settings.cornerTopRight}`;
			case "top-left":
				return `↖ ${t.settings.cornerTopLeft}`;
			default:
				return "";
		}
	};
